...
public class peer {
	...
	void running() {
		try {
			...
			totalChunk = Integer.parseInt(MESSAGE);
			...
			if (MESSAGE.equals("START")) {
				while (!(MESSAGE = (String)in.readObject()).equals("END")) {
					...
					ownedArray1 = getOwnedArray();
					ownedArray1.add(Integer.parseInt(file_name));
					storeArray(ownedArray1);
				...
				message = "All chunks are received.";
				sendMessage(message);
			}
	...
	public void sendMessage(String msg) {
		...
		private static class Handler extends Thread {
			...
			//} catch (IOException ioException) {
				//System.out.println("Disconnect with peer " + no2);
			//}
		//}
		...
		public static void storeArray(ArrayList<Integer> x){
			lock.lock();
			try {
